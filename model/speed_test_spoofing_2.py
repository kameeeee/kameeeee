...
BrakeData = np.array( [BrakeData[v] for v in xrange(0, BrakeData.size, 50)] )/100

SpeedData = SpeedData[:1100]
BrakeData = BrakeData[:1100]

#学習済みモデルのロード
model = RNN_model.RNN_model()
serializers.load_npz("./Linear_1_LSTM_1_Linear_1/50node_2/50node_2.model", model)
...
plt.plot(SpeedData[1:]*100, ':b', label='speed data')
plt.legend(loc='upper right', fontsize=16)
plt.tick_params(labelsize=18)
ax2 = fig.add_subplot(212)
plt.title('brake data', fontsize=22)
plt.xlabel('Number of Frames', fontsize=22)
plt.ylabel('brake amount', fontsize=22)
ax2.plot(BrakeData[1:]*100, 'g', label='brake amount')
