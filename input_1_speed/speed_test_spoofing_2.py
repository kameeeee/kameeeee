...
for i in xrange(SpeedData[3950:].size-1):
    ...
    if answer == -1:
        s = Variable( np.asarray( [ [ SpeedData[i+3950] ] ], dtype=np.float32) )
    else:
        s = Variable( np.asarray( [ [ answer ] ], dtype=np.float32) )
        
    pre = model.fwd(s).data[0][0]
    nextdata[1] = SpeedData[i+3951]
...
plt.xlabel('time')
plt.ylabel('speed(km/h)')
ax1.plot(SpeedData[3951:], 'b', label='test data(speed)')
fig, ax2 = plt.subplots()
plt.title('output')
plt.xlabel('time')
plt.ylabel('speed(km/h)')
ax2.plot(answer_speed, 'm', label='output')
fig, ax3 = plt.subplots()
plt.title('attack data')
plt.xlabel('time')
plt.ylabel('speed(km/h)')
ax3.plot(attacker, 'r', label='attack data')
plt.show()
'''
