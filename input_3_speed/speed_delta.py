...
serializers.load_npz("./learned/7_Linear1_100_LSTM2_100_Linear1_100_interval_50_delta300_10.model", model)
...
plt.tight_layout()
ax2.plot(xrange(SpeedData[3661:].size-1), SpeedData[3662:])

while epoch <= n_epoch:
    [source, target] = make_minibatch(SpeedData[:3660].size, delta, Minibatch_size, SpeedData[:3660], BrakeData[:3660], AccelatorData[:3660])
    model.reset_state()
    model.zerograds()
    loss = 0
    ...
    if epoch % 100 == 0:
        ...
        if epoch % 100 == 0:
            ...
            for i in xrange(SpeedData[3661:].size-1):
                s = Variable( xp.asarray( [ [ SpeedData[i+3661], BrakeData[i+3661], AccelatorData[i+3661] ] ], dtype = xp.float32 ) )
                t = Variable( xp.asarray( [ [ SpeedData[i+3662] ] ], dtype = xp.float32 ) )
                ans = model.fwd(s)
            ...
            validation_loss /= SpeedData[3661:].size
        ...
        if epoch == 0:
            ...
            lines3, = ax2.plot(xrange(SpeedData[3661:].size-1), answer_speed)
            
        else:
            lines.set_data(line_x, line_y)
            if val_flag == 1:
                lines2.set_data(line_x_val, line_y_val)
                lines3.set_data(xrange(SpeedData[3661:].size-1), answer_speed)
...
plt.savefig("./png/7_Linear1_100_LSTM2_100_Linear1_100_interval_50_delta300_10.png")

if gpu_flag >= 0:
    model.to_cpu()

serializers.save_npz("./learned/7_Linear1_100_LSTM2_100_Linear1_100_interval_50_delta300_10.model", model)
serializers.save_npz("./learned/7_Linear1_100_LSTM2_100_Linear1_100_interval_50_delta300_10.state", optimizer)
